...
You (Claude) are the orchestrator. Workers run in herdr panes, each a real `claude` CLI instance pointed at a cockpit-cliproxy gateway instance (:1456-1459) fronting SenseNova. The verifier layer auto-checks worker output and redoes on FAIL without spending your tokens.
...
3. Before first use in a session: `powershell D:\devloop\workSpace\app_ZCode\herd-flows\runner\herdr-pool.ps1 -Ensure` (starts herdr session `subclaw` if down, writes worker settings, probes gateway ports).
4. If a gateway port reports DOWN, tell the user to `docker compose -f "D:\Docker Project\cockpit-tools\compose.subclaw-pool.yml" --env-file "D:\Docker Project\cockpit-tools\.env" up -d`.
...
| `deepseek` | deepseek-v4-flash | 1457 | reasoning, audit, cross-file analysis |
...
- Verifier = a second worker on `deepseek` profile.
- Prompt template in `D:/devloop/workSpace/app_ZCode/herd-flows/runner/config.json` `verifier.promptTemplate`.
