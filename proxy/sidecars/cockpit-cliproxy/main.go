...
func resolveLaunchFiles(configPath string, manifestPath string) (string, string, func(), error) {
	configPath = strings.TrimSpace(configPath)
	manifestPath = strings.TrimSpace(manifestPath)
	if configPath != "" && manifestPath != "" {
		return configPath, manifestPath, func() {}, nil
	}
	if configPath != "" || manifestPath != "" {
		return "", "", func() {}, fmt.Errorf("missing --config or --manifest")
	}
	return writeEnvBootstrapLaunchFiles()
}

func writeEnvBootstrapLaunchFiles() (string, string, func(), error) {
	upstreamAPIKey := firstEnv("COCKPIT_UPSTREAM_API_KEY", "OPENAI_API_KEY")
	if strings.TrimSpace(upstreamAPIKey) == "" {
		return "", "", func() {}, fmt.Errorf("missing --config/--manifest; set COCKPIT_UPSTREAM_API_KEY or OPENAI_API_KEY for env bootstrap")
	}

	workDir, err := os.MkdirTemp("", "cockpit-cliproxy-")
	if err != nil {
		return "", "", func() {}, err
	}
	...
	host := envDefault("COCKPIT_PROXY_HOST", "0.0.0.0")
	port := envIntDefault("COCKPIT_PROXY_PORT", 1456)
	clientAPIKey := envDefault("COCKPIT_CLIENT_API_KEY", "cockpit-local-key")
	upstreamBaseURL := firstEnv("COCKPIT_UPSTREAM_BASE_URL", "OPENAI_BASE_URL")
	if strings.TrimSpace(upstreamBaseURL) == "" {
		upstreamBaseURL = "https://api.openai.com/v1"
	}
	upstreamModel := firstEnv("COCKPIT_UPSTREAM_MODEL", "OPENAI_MODEL")
	if strings.TrimSpace(upstreamModel) == "" {
		upstreamModel = "gpt-5.5"
	}
	upstreamModels := envListDefault("COCKPIT_UPSTREAM_MODELS", []string{upstreamModel})
	if !stringSliceContainsFold(upstreamModels, upstreamModel) {
		upstreamModels = append([]string{upstreamModel}, upstreamModels...)
	}
	wireAPI := normalizeProviderGatewayWireAPI(envDefault("COCKPIT_UPSTREAM_WIRE_API", "responses"))
	debugLogs := envBoolDefault("COCKPIT_DEBUG_LOGS", false)
	disableImageGeneration := envBoolDefault("COCKPIT_DISABLE_IMAGE_GENERATION", false)
	sessionAffinity := envBoolDefault("COCKPIT_SESSION_AFFINITY", true)
	providerLabel := envDefault("COCKPIT_PROVIDER_LABEL", "Docker Provider")
	modelPrefix := strings.Trim(strings.TrimSpace(os.Getenv("COCKPIT_MODEL_PREFIX")), "/")
	...
	configValue := map[string]any{
		...
		"auth-dir":                 authDir,
		"debug":                    debugLogs,
		"api-keys":                 []string{clientAPIKey},
		"request-log":              false,
		"logging-to-file":          false,
		"commercial-mode":          true,
		"ws-auth":                  true,
		"disable-image-generation": disableImageGeneration,
		"request-retry":            1,
		"streaming": map[string]any{
			"keepalive-seconds":            15,
			"bootstrap-retries":            1,
			"stream-open-timeout-ms":       60000,
			"stream-idle-timeout-ms":       120000,
			"image-stream-open-timeout-ms": 60000,
			"image-stream-idle-timeout-ms": 180000,
			"stream-open-max-attempts":     1,
		},
		...
		"routing": map[string]any{
			...
			"session-affinity-ttl": "3600000ms",
		},
		"codex-header-defaults": map[string]any{
			"user-agent":    "codex-tui/0.135.0",
			"beta-features": "responses_websockets=2026-02-06",
		},
	}
	...
	manifestValue := map[string]any{
		"apiKeys": []map[string]any{{
			...
			"providerGateway": map[string]any{
				"baseUrl":        upstreamBaseURL,
				"apiKey":         upstreamAPIKey,
				"upstreamModel":  upstreamModel,
				"upstreamModels": upstreamModels,
				"wireApi":        wireAPI,
				"supportsVision": envBoolDefault("COCKPIT_SUPPORTS_VISION", false),
			},
			"accountIds":     []string{},
		...
		"accounts":           []any{},
		"modelIds":           upstreamModels,
		"modelAliases":       []any{},
		"excludedModels":     envListDefault("COCKPIT_EXCLUDED_MODELS", nil),
		"routingStrategy":    "auto",
		"customRoutingRules": []any{},
		"debugLogs":          debugLogs,
	}

	configPath := filepath.Join(workDir, "config.json")
	manifestPath := filepath.Join(workDir, "manifest.json")
	if err := writeJSONFile(configPath, configValue); err != nil {
		cleanup()
		return "", "", func() {}, err
	}
	if err := writeJSONFile(manifestPath, manifestValue); err != nil {
		cleanup()
		return "", "", func() {}, err
	}
	return configPath, manifestPath, cleanup, nil
}
...
func main() {
	...
	emitter := &eventEmitter{}
	resolvedConfigPath, resolvedManifestPath, cleanup, err := resolveLaunchFiles(*configPath, *manifestPath)
	if err != nil {
